...
class AAAengine:
    ...
    def readableStatus(self) -> str:
        ...
        status = 'CurrentTurn: ' + str(self.currentPlayer) + " Phase: " + str(self.currentPhase) + "\n"
    ...
    def moveUnit(self, moveFrom: Territory, detailedUnit: DetailedUnit, destinationTransport: DestinationTransport):
        if self.verbose:
            print("Moving unit: " + str(detailedUnit) + " from: " + str(moveFrom) + " to: " + str(destinationTransport))
                    ...
                    if unitQuantity.detailedUnit.unitType.bombard > 0:
                        if battleShipsToSkip > 0:
                            ...
                            if random.randint(1,6) <= bombardValue:
                                if player.isHuman:
                                    print("bombard hit!")
                                self.removeCasualty(unloadingConnection.destination, unitQuantities=enemyUnits)
        for unloadingConnection in unitQuantitiesForConnection:
